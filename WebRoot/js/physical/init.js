...
var defaultTracks = getQueryString("tracks"); // used to get default tracks
	...
	function toggleTrack(category,track,index,genomeflag){
		...
		var checktracklist = $("#trackidlist input:checkbox:checked"); // 
				    ...
				    for(i=0;i<atoms.length;i++){
							...
							for(j=0;j<data.genelist.length;j++){
									...
									if( (atomprop.chr == genomepos[0] && !(atomprop.end < genomepos[1]|| atomprop.start > genomepos[2]) ) && atomprop.chr == geneobj.chrom){	
										...
										if( cyradius >0 ){
												console.log("============focusgene="+focusgene+",synonym="+geneobj.synonym);
												if(f_count <0 && focusgene.indexOf(geneobj.synonym) > -1){
													f_count = 0 ;
													focusAtom = p_atom;
													console.log("===========focus atom=="+focusAtom.serial);
												}
...
var seltracks_number = $("#trackidlist input:checkbox:checked").length;
...
function ShowTrackColorMap(category,trackkey,trackcolor,glyph){
	...
	/*else{
		...
		if(glyph == "gene"){
			var spantext="<span style='padding-left:5px;' id='hm"+trackkey+"'><img src='/circosweb/images/tabClose.png' onclick='untoggleTrack(\""+category+"\",\""+trackkey+"\")'>"+trackkey;
			if(trackcolor == undefined ||trackcolor == null || trackcolor.length == 0){
				spantext += "<img src='/circosweb/images/hotpink.png' />Forward&nbsp;<img src='/circosweb/images/cornflowerblue.png' />Reverse</span>";
			}else{
				spantext += "<img src='/circosweb/images/tad"+trackcolor+".gif' /></span>";
			}
			
			
		}else if(glyph == "dashedline"){
			var spantext="<span style='padding-left:5px;' id='hm"+trackkey+"'><img src='/circosweb/images/tabClose.png' onclick='untoggleTrack(\""+category+"\",\""+trackkey+"\")'>"+trackkey;
			spantext += "<img src='/circosweb/images/dashedline.gif' /></span>";
		}else{
			var spantext="<span style='padding-left:5px;' id='hm"+trackkey+"'><img src='/circosweb/images/tabClose.png' onclick='untoggleTrack(\""+category+"\",\""+trackkey+"\")'>"+trackkey+"<img src='/circosweb/images/"+trackcolor+".png' /></span>";
		}
		if(td != null ){
			td.append(spantext);
		}	
...
function ShowTADColorMap(category,trackkey,trackindex,idflag,trackcolor){
	...
	if(idflag1 == 0){
			spantext="<span style='padding-left:5px;' id='hm"+trackkey+"' ><img src='/circosweb/images/tabClose.png' onclick='untoggleTrack(\""+category+"\",\""+trackkey+"\")'>"+trackkey+"</span>" ;
			td.append(spantext);
			
	}
	var hhobj = $("#hm"+trackkey);
	if(hhobj.length > 0 ){
		spantext =" "+trackindex+"<img src='/circosweb/images/tad"+trackcolor+".gif' />";
